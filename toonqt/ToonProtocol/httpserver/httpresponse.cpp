...
void HttpResponse::writeHeaders()
{
    foreach(QString name, m_headers.keys())
    {
        ...
        writeHeader(name.toStdString().c_str/*toAscii*/(), value/*.toAscii()*/);
    }
...
void HttpResponse::writeHead()
{
    ...
    m_connection->write(QString("HTTP/1.1 %1 %2\r\n").arg(status).arg(STATUS_CODES[status]).toStdString().c_str/*.toAscii*/());
...
void HttpResponse::end(const QString &data)
{
    ...
    emit done();
    deleteLater();*/
	end(QByteArray(data.toStdString().c_str())); // data.toAscii()
}
