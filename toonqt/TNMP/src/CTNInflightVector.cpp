...
{
    std::unique_lock<std::mutex> unilock(m_mutexInflight);
    TNMPLOG("removeInflightMutex:"<<msgId);
...
{
    int size = 0;
    TNMPLOG("before removed inflight:"<<m_vctInflight.size());
    std::vector<InflightMessage*>::iterator itTmp = m_vctInflight.begin();
    while (itTmp != m_vctInflight.end()) {
        //must define tmpMsg;
        InflightMessage* tmpMsg = *itTmp;
        if (tmpMsg->req->msg_id == msgId){
           itTmp = m_vctInflight.erase(itTmp);
            
            size = tmpMsg->size;
            delete tmpMsg;
            
            TNMPLOG("find msg and removed:"<<msgId);
            break;
        }
        itTmp ++;
    }
    
    TNMPLOG("after removed inflight:"<<m_vctInflight.size());
    return size;
}
...
void CTNInflightVector::checkInflight(bool isOffline, std::function<void(bool isResend, InflightMessage*)> callback)
{
    ...
    while (itTmp != m_vctInflight.end()) {
        ...
        if ((*itTmp)->retryTimes >= RETRY_MAXTIMES) {
            vctRemoved.push_back((*itTmp)->req->msg_id);
            
            TNMPLOG("Send failed:"<<(*itTmp)->req->msg_id);
...
//            cout <<"don't check inflight"<<endl;
        ...
        else if (now - (*itTmp)->req->timestamp > RESEND_TIMER){
//            cout<<"checkInflight:"<<now<<" timestamp:"<<(*itTmp)->req->timestamp<<endl;;
            (*itTmp)->req->timestamp = now;
            (*itTmp)->retryTimes ++;
            
            TNMPLOG("Resend msg:"<<(*itTmp)->req->msg_id<<" retrytimes:"<<(*itTmp)->retryTimes);
             callback(true, *itTmp);
        }
        itTmp ++;
    }
