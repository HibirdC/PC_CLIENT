...
CSSLSocket::~CSSLSocket()
{
    TNMPLOG("CSSLSocket::~CSSLSocket ENTER:"<<m_socket);
    if (_ssl != nullptr){
        SSL_free(_ssl);
        _ssl = nullptr;
    }
    TNMPLOG("CSSLSocket::~CSSLSocket 2");

    if (_ssl_ctx != nullptr){
        TNMPLOG("CSSLSocket::~CSSLSocket 21");
        SSL_CTX_free(_ssl_ctx);
        _ssl_ctx = nullptr;
    }
    
    TNMPLOG("CSSLSocket::~CSSLSocket EXIT");
}
...
int CSSLSocket::connectTLS(){
    TNMPLOG("ENTER connectTLS");
#if 0
    int ret = SSL_connect(_ssl);
    if(ret != 1){
        //    SSL_ERROR_NONE
        int errcode = SSL_get_error(_ssl, ret);
        TNMPLOG("connectTLS SSL_connect FAILED ret:"<<ret<<"  ERR_get_error():"<<errcode);
    ...
    while (true)
    {
        ...
        switch(SSL_get_error(_ssl, ret))
        {
            case SSL_ERROR_WANT_WRITE:
                ...
                TNMPLOG("SSL_connect SSL_ERROR_WANT_WRITE!");
                break;
            case SSL_ERROR_WANT_READ:
                sel.read_FD_SET(this->socketFD());
                /* Wait for epoll/select to return */
                TNMPLOG("SSL_connect SSL_ERROR_WANT_READ!");
                break;
        }
     
        TNMPLOG("before CTNSocketSelect select:"<<ret);
        sel.exception_FD_SET(this->socketFD());
        ret = sel.select(5000);
        TNMPLOG("after CTNSocketSelect select:"<<ret);
        if (ret == 0){
            TNMPLOG("SSL_connect time out!");
            return TMTP_ERR_TLS;
        }
        if (sel.isException()) {
            return TMTP_ERR_TLS;
...
{
    TNMPLOG("ENTER CSSLSocket::connectToHost:"<<host<<" port:"<<port);
    
    //socket connect retry 3times;
    int ret = 0;
    ret = CTcpSocket::connectToHost(host, port);
    if (ret != 0){
        TNMPLOG("CTcpSocket::connectToHost failed:"<<ret);
        return ret;
    }
...
//    SSL_set_tlsext_host_name(_ssl, host);
    ...
    ret = this->connectTLS();
    TNMPLOG("Exit connectToHost::connectToHost:"<<ret);
...
int CSSLSocket::sendData(const char *buf, size_t len)
{
    ...
    if(ret <= 0){
        ...
        if (err == SSL_ERROR_WANT_READ
            ...
            ret = TMTP_ERR_SUCCESS;
        }
        else {
            TNMPLOG("SSL_write error ret:"<<ret<<" err:"<<err);
...
int CSSLSocket::recvData(char *buf, size_t len)
{
    ...
    if(ret <= 0){
        ...
        if (err == SSL_ERROR_WANT_READ
            ...
            TNMPLOG("SSL_read error ret:"<<ret<<" err:"<<err);
            ret = TMTP_ERR_SOCK_CLOSE;
        }
    }
