...
TNHttpObtainGroupChatsByUserId::TNHttpObtainGroupChatsByUserId()
{
...
void TNHttpObtainGroupChatsByUserId::requestFinished(QNetworkReply* reply, const QByteArray data, const int statusCode)
{
	if (statusCode == 200) 
	{
...
void TNHttpObtainGroupChatsByUserId::GetJson(std::function<void(bool, QJsonObject)> callback, const TNPluginParamPtr &pluginParam, 
	const QString & version)
{
	this->checkCallback = callback;
	// »ù±¾ URL
	QString baseUrl = pluginParam->str_api_new_groupchat_systoon_com + URL_OBTAIN_GROUPCHATS_BYUSERID;
	QUrl url(baseUrl);
	QUrlQuery query;
	query.addQueryItem("version", version);
	url.setQuery(query);
    qInfo() << "[NetWorkAPI][TNHttpObtainGroupChatsByUserId] url" << url;
