...
TNHttpGetListStaffCardByUserId::TNHttpGetListStaffCardByUserId()
{
...
void TNHttpGetListStaffCardByUserId::requestFinished(QNetworkReply* reply, const QByteArray data, const int statusCode)
{
	if (statusCode == 200) 
	{
...
void TNHttpGetListStaffCardByUserId::GetJson(std::function<void(bool, QJsonObject)> callback, const TNPluginParamPtr &pluginParam,
	const QString & version)
{
	this->checkCallback = callback;
	// »ù±¾ URL
	QString baseUrl = pluginParam->str_new_org_systoon_com + URL_LIST_STAFF_CARD_BYUSERID;
	QUrl url(baseUrl);
	QUrlQuery query;
	query.addQueryItem("userId", pluginParam->strTnUserID);
	query.addQueryItem("version", version);
	url.setQuery(query);
    qInfo() << "[NetWorkAPI][TNHttpGetListStaffCardByUserId]-url" << url;
