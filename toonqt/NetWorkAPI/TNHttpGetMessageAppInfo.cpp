...
TNHttpGetMessageAppInfo::TNHttpGetMessageAppInfo()
{
...
void TNHttpGetMessageAppInfo::requestFinished(QNetworkReply* reply, const QByteArray data, const int statusCode)
{
	if (statusCode == 200) 
	{
...
void TNHttpGetMessageAppInfo::GetAppNoticeById(std::function<void(bool, QJsonObject)> callback, const TNPluginParamPtr &pluginParam,const QString& appId)
{
	this->checkCallback = callback;
	// 基本 URL
	QString baseUrl = pluginParam->str_api_app_systoon_com + URL_GETAPPNOTICE_BYID;
	QUrl url(baseUrl);
	QUrlQuery query;
	query.addQueryItem("appId", appId);
	url.setQuery(query);
    qInfo() << "[NetWorkAPI][TNHttpGetMessageAppInfo]-url" << url;
