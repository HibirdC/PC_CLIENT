...
TNHttpGetMyGroup::TNHttpGetMyGroup()
{
...
void TNHttpGetMyGroup::requestFinished(QNetworkReply* reply, const QByteArray data, const int statusCode)
{
	if (statusCode == 200)
	{
...
void TNHttpGetMyGroup::GetJson(std::function<void(bool, QJsonObject)> callback, const TNPluginParamPtr &pluginParam,
	const QString & version, const QString &myFeed)
{
	this->checkCallback = callback;
	// 基本 URL
	QString baseUrl = pluginParam->str_new_group_systoon_com + URL_GROUP_CARD_GETMYGROUP;
	QUrl url(baseUrl);
	QUrlQuery query;
	query.addQueryItem("userId", pluginParam->strTnUserID);
	if (!myFeed.isEmpty())
	{
		query.addQueryItem("cardFeedId", myFeed);
	}
	query.addQueryItem("version", version);
	url.setQuery(query);
	qInfo() <<"[NetWorkAPI][TNHttpGetMyGroup] url:"<< url;
