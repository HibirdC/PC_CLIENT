...
TNHttpGetListStaffCard::TNHttpGetListStaffCard()
{
...
void TNHttpGetListStaffCard::requestFinished(QNetworkReply* reply, const QByteArray data, const int statusCode)
{
	if (statusCode == 200) 
	{
...
void TNHttpGetListStaffCard::GetJson(std::function<void(bool, QJsonObject)> callback, const TNPluginParamPtr &pluginParam, 
	const QString & feedIDs)
{
	this->checkCallback = callback;
	// 基本 URL
	QString baseUrl = pluginParam->str_new_org_systoon_com + URL_LIST_STAFF_CARD;
	QUrl url(baseUrl);
	QUrlQuery query;
	query.addQueryItem("feedIdStr", feedIDs);
	url.setQuery(query);
    qInfo() << "[NetWorkAPI][TNHttpGetListStaffCard]-url="  << url;
