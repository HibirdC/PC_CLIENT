...
TNCHNHelper::TNCHNHelper(QObject *parent)
	: QObject(parent)
{

}

TNCHNHelper::~TNCHNHelper()
{

}
bool TNCHNHelper::In(wchar_t start, wchar_t end, wchar_t code)
{
...
char TNCHNHelper::Convert(int n)
{
	/*
	* GB2312码范围 (B0A1-F7FE)
...
QString TNCHNHelper::GetFirstLetter(const QString &src)
{
	wchar_t wchr= 0;
	QString firstLetter;
	if (src.size() > 0)
...
QString TNCHNHelper::GetFirstLetters(const QString &src)
{
...
QString TNCHNHelper::GetPinyins(const QString &text)
{
#ifdef Q_OS_MAC
    ...
    for (int i = 0; i<text.length(); i++)
    {
        ...
        pinyins += GetPinyin(code).toLower();
        if (!pinyins.isEmpty())
        {
			pinyins += SPACE_STR;
        }
...
#ifdef Q_OS_MAC
	QTextCodec *codec2 = QTextCodec::codecForName("System");
	QTextCodec::setCodecForLocale(codec2);
#endif
    qDebug() << "[TNCHNHelper] py:" << pinyins;
    return pinyins;
}

/*
汉字区位码表：https://wenku.baidu.com/view/29d687075ef7ba0d4b733b28.html?re=view
*/
QString TNCHNHelper::GetPinyin(int code)
{
	QString pinyin;
	switch (code)
	{
