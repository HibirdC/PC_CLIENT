...
void WavWriter::write_string(struct wav_writer* ww, const char *str) {
	fputc(str[0], ww->wav);
	fputc(str[1], ww->wav);
	fputc(str[2], ww->wav);
	fputc(str[3], ww->wav);
...
void WavWriter::write_int32(struct wav_writer* ww, int value) {
...
void WavWriter::write_int16(struct wav_writer* ww, int value) {
	fputc((value >> 0) & 0xff, ww->wav);
	fputc((value >> 8) & 0xff, ww->wav);
}

void WavWriter::write_header(struct wav_writer* ww, int length) {
...
void* WavWriter::wav_write_open(const char *filename, int sample_rate, int bits_per_sample, int channels) {
...
void WavWriter::wav_write_close(void* obj) {
...
void WavWriter::wav_write_data(void* obj, const unsigned char* data, int length) {
	struct wav_writer* ww = (struct wav_writer*) obj;
	if (ww->wav == NULL)
		return;
